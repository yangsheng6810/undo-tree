  ...
  (undo-tree-node
   ...
   (:constructor undo-tree-make-node-backwards
		  ...
		  &optional redo
                  ...
                  (timestamp (current-time))
                  (branch 0)))
   (:copier undo-tree-node-make-copy)
   )
  previous next undo redo timestamp branch meta-data)


;; ;; automatically generated by cl-defstruct
...
;; ;; automatically generated by cl-defstrut
...
(defun undo-tree-clean-GCd-elts (tree &optional clean-all)
  ;; Clear visualizer data below NODE.
  ;; Clean all markers when CLEAN-ALL is t
  (let ((temp-object-pool (undo-tree-object-pool tree)))
    (when clean-all
    ...
    (undo-tree-mapc
     (lambda (node)
       (when (undo-tree-node-undo node)
         (undo-tree/debug-message 10 "temp before updating undo: %S" (undo-tree-node-undo node))
         (undo-tree--update-undo tree node)
         (undo-tree/debug-message 10 "temp after: %S" (undo-tree-node-undo node))
         )
       (when (undo-tree-node-redo node)
         (undo-tree/debug-message 10 "temp before updating redo: %S" (undo-tree-node-redo node))
         (undo-tree--update-redo tree node)
         (undo-tree/debug-message 10 "temp after: %S" (undo-tree-node-undo node))))
     (undo-tree-root tree))
    (when clean-all
      (setf (undo-tree-object-pool tree)
...
(defun undo-tree--sorted-next (node)
  ;; field NEXT of undo-tree-node is a vector, so we need a mapcar
  ;; to get a list
  (sort (mapcar 'identity (undo-tree-node-next node))
        (lambda (a b)
...
(defun undo-tree-rebuild-list-from-tree ()
  ...
  (unless (eq buffer-undo-list t)
	...
	(push (undo-tree--sorted-next (caar stack))
	      stack)
	...
	(while (or
                ...
                (not (eq (car (nth 1 stack))
                         (undo-tree-current buffer-undo-tree))))
          ;; (car stack) is contains the list of unvisited children of current
          ;; node, i.e. (cadr stack). If it is empty, then we are ready to go up
	  ...
	  (if (car stack)
		...
		(push (undo-tree--sorted-next (caar stack))
            ...
            ;; empty list of children, i.e. (car stack), and go up
	    (pop stack)
	    (setq buffer-undo-list
...
(defun undo-tree-undo-1 (&optional arg preserve-redo preserve-timestamps)
  ...
  (let ((undo-in-progress t)
	...
	(user-error "No further undo information for region"))
      ...
      (undo-tree--update-undo buffer-undo-tree current)
	...
	(set-marker-insertion-type pos t))
	  ...
	  (progn
	    (undo-tree-list-pop-changeset)
            (undo-tree--update-redo buffer-undo-tree current))
	...
	;; existing entry if one already exists
        (undo-tree--update-redo buffer-undo-tree current (undo-tree-list-pop-changeset 'discard-pos)))
...
(defun undo-tree--print-recursive (tree)
  (let ((index 0)
    ...
    (undo-tree-clean-GCd-elts tree t)
    ...
    (with-temp-buffer
      ...
      ;; print a new line when necessary
...
(defun undo-tree--save-history-old (filename)
  (unwind-protect
      (progn
  ...
  (when (and buffer-undo-tree (not (eq buffer-undo-tree t)))
	...
	(setq filename
      ...
      (when (or (not (file-exists-p filename))
        ...
        (undo-tree-clean-GCd-elts buffer-undo-tree)
	(undo-tree--save-history-old filename)))))
